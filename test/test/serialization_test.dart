import 'package:blockchain_utils/blockchain_utils.dart';
import 'package:cosmos_sdk/cosmos_sdk.dart';
import 'package:cosmos_sdk/proto_messages/cosmos/bank/v1beta1/src/tx.dart';
import 'package:cosmos_sdk/proto_messages/cosmos/base/v1beta1/src/coin.dart';
import 'package:cosmos_sdk/proto_messages/cosmos/tx/signing/v1beta1/src/signing.dart';
import 'package:cosmos_sdk/proto_messages/cosmos/tx/v1beta1/src/tx.dart';
import 'package:test/test.dart';

void main() {
  group("serialization", () {
    _fee();
    _signerInfo();
    _authInfo();
    _msgSend();
    _txBody();
    _signDoc();
    _txRaw();
  });
}

void _fee() {
  test("fee", () {
    final fee = Fee(
      amount: [Coin(denom: "uatom", amount: "500")],
      gasLimit: BigInt.from(200000),
    );
    expect(
      BytesUtils.toHexString(fee.toBuffer()),
      "0a0c0a057561746f6d120335303010c09a0c",
    );
    final decode = Fee.deserialize(fee.toBuffer());
    expect(fee.toBuffer(), decode.toBuffer());
  });
}

void _signerInfo() {
  test("signer info", () {
    final sigInfo = SignerInfo(
      publicKey:
          CosmosSecp256K1PublicKey.fromHex(
            "0312b4a708eaf861f52a8e934716d24b3ba3b13cf3acee3145aff924507a6e3f77",
          ).toAny(),
      modeInfo: const ModeInfo(
        single: ModeInfoSingle(mode: SignMode.signModeDirect),
      ),
      sequence: BigInt.zero,
    );
    final decode = SignerInfo.deserialize(sigInfo.toBuffer());
    expect(sigInfo.toBuffer(), decode.toBuffer());
    expect(
      BytesUtils.toHexString(sigInfo.toBuffer()),
      "0a460a1f2f636f736d6f732e63727970746f2e736563703235366b312e5075624b657912230a210312b4a708eaf861f52a8e934716d24b3ba3b13cf3acee3145aff924507a6e3f7712040a020801",
    );
  });
}

void _authInfo() {
  test("auth info", () {
    final authInfo = AuthInfo(
      signerInfos: [
        SignerInfo(
          publicKey:
              CosmosSecp256K1PublicKey.fromHex(
                "0312b4a708eaf861f52a8e934716d24b3ba3b13cf3acee3145aff924507a6e3f77",
              ).toAny(),
          modeInfo: const ModeInfo(
            single: ModeInfoSingle(mode: SignMode.signModeDirect),
          ),
          sequence: BigInt.zero,
        ),
      ],
      fee: Fee(
        amount: [Coin(denom: "uatom", amount: "500")],
        gasLimit: BigInt.from(200000),
      ),
    );
    final decode = AuthInfo.deserialize(authInfo.toBuffer());
    expect(authInfo.toBuffer(), decode.toBuffer());
    expect(
      BytesUtils.toHexString(authInfo.toBuffer()),
      "0a4e0a460a1f2f636f736d6f732e63727970746f2e736563703235366b312e5075624b657912230a210312b4a708eaf861f52a8e934716d24b3ba3b13cf3acee3145aff924507a6e3f7712040a02080112120a0c0a057561746f6d120335303010c09a0c",
    );
  });
}

void _msgSend() {
  test("msg send", () {
    final message = MsgSend(
      fromAddress: "cosmos1htg7dmhelazdsmuwm9ngtg0tpe82006uqdw9nj",
      toAddress: "cosmos1qhslf0sx2fegltfqq0p5j6etmdznjgfnm2j6nc",
      amount: [Coin(denom: "uatom", amount: "1000000")],
    );
    final decode = MsgSend.deserialize(message.toBuffer());
    expect(message.toBuffer(), decode.toBuffer());
    expect(
      BytesUtils.toHexString(message.toBuffer()),
      "0a2d636f736d6f733168746737646d68656c617a64736d75776d396e67746730747065383230303675716477396e6a122d636f736d6f73317168736c66307378326665676c746671713070356a3665746d647a6e6a67666e6d326a366e631a100a057561746f6d120731303030303030",
    );
  });
}

void _txBody() {
  test("txbody", () {
    final message = MsgSend(
      fromAddress: "cosmos1htg7dmhelazdsmuwm9ngtg0tpe82006uqdw9nj",
      toAddress: "cosmos1qhslf0sx2fegltfqq0p5j6etmdznjgfnm2j6nc",
      amount: [Coin(denom: "uatom", amount: "1000000")],
    );
    final txbody = TxBody(messages: [message.toAny()]);
    final decode = TxBody.deserialize(txbody.toBuffer());
    expect(txbody.toBuffer(), decode.toBuffer());
    expect(
      BytesUtils.toHexString(txbody.toBuffer()),
      "",
    );
  });
}

void _signDoc() {
  test("signDoc", () {
    final authInfo = AuthInfo(
      signerInfos: [
        SignerInfo(
          publicKey:
              CosmosSecp256K1PublicKey.fromHex(
                "0312b4a708eaf861f52a8e934716d24b3ba3b13cf3acee3145aff924507a6e3f77",
              ).toAny(),
          modeInfo: const ModeInfo(
            single: ModeInfoSingle(mode: SignMode.signModeDirect),
          ),
          sequence: BigInt.zero,
        ),
      ],
      fee: Fee(
        amount: [Coin(denom: "uatom", amount: "500")],
        gasLimit: BigInt.from(200000),
      ),
    );
    final message = MsgSend(
      fromAddress: "cosmos1htg7dmhelazdsmuwm9ngtg0tpe82006uqdw9nj",
      toAddress: "cosmos1qhslf0sx2fegltfqq0p5j6etmdznjgfnm2j6nc",
      amount: [Coin(denom: "uatom", amount: "1000000")],
    );
    final txbody = TxBody(messages: [message.toAny()]);
    final SignDoc signDoc = SignDoc(
      bodyBytes: txbody.toBuffer(),
      authInfoBytes: authInfo.toBuffer(),
      chainId: "theta-testnet-001",
      accountNumber: BigInt.from(747363),
    );
    final decode = SignDoc.deserialize(signDoc.toBuffer());
    expect(signDoc.toBuffer(), decode.toBuffer());
    expect(
      BytesUtils.toHexString(signDoc.toBuffer()),
      "",
    );
  });
}

void _txRaw() {
  test("txRaw", () {
    final authInfo = AuthInfo(
      signerInfos: [
        SignerInfo(
          publicKey:
              CosmosSecp256K1PublicKey.fromHex(
                "0312b4a708eaf861f52a8e934716d24b3ba3b13cf3acee3145aff924507a6e3f77",
              ).toAny(),
          modeInfo: const ModeInfo(
            single: ModeInfoSingle(mode: SignMode.signModeDirect),
          ),
          sequence: BigInt.zero,
        ),
      ],
      fee: Fee(
        amount: [Coin(denom: "uatom", amount: "500")],
        gasLimit: BigInt.from(200000),
      ),
    );
    final message = MsgSend(
      fromAddress: "cosmos1htg7dmhelazdsmuwm9ngtg0tpe82006uqdw9nj",
      toAddress: "cosmos1qhslf0sx2fegltfqq0p5j6etmdznjgfnm2j6nc",
      amount: [Coin(denom: "uatom", amount: "1000000")],
    );
    final txbody = TxBody(messages: [message.toAny()]);
    final txRaw = TxRaw(
      bodyBytes: txbody.toBuffer(),
      authInfoBytes: authInfo.toBuffer(),
      signatures: [
        BytesUtils.fromHexString(
          "4409cfd68f922f0aad913af1a415ea5b24550784f5a6f11408ccc18678747bf14a327fce0fdc1c5c99f7011065670dbf5ae187ed8cb96708cec9d3df3af92a1b",
        ),
      ],
    );
    final decode = TxRaw.deserialize(txRaw.toBuffer());
    expect(txRaw.toBuffer(), decode.toBuffer());
    expect(
      BytesUtils.toHexString(txRaw.toBuffer()),
      "",
    );
  });
}
