...
abstract class InitiaService<T extends CosmosMessage>
  ...
  static T? fromJson<T extends ServiceMessage>(
    ...
    final ServiceMessage? service = switch (type) {
      InitiaMoveV1Types.msgExecute => InitiaMoveV1MsgExecute.fromJson(json),
      _ => null
    } as ServiceMessage?;

    if (service == null) return null;
    if (service is! T) {
      throw DartCosmosSdkPluginException("Invalid Initia Service.", details: {
        "excepted": "$T",
  ...
  static T? deserialize<T extends ServiceMessage>(
    ...
    final ServiceMessage? service = switch (type) {
      InitiaMoveV1Types.msgExecute => InitiaMoveV1MsgExecute.deserialize(bytes),
      _ => null
    } as ServiceMessage?;
