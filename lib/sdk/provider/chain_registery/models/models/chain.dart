...
class CCREndpoint {
  ...
  CCRApi? toSupportedApi(CosmosProviderApi api) {
    final uri = Uri.tryParse(address);
    if (uri == null) return null;
    return CCRApi(url: address, api: api);
  }
}
...
class CCRApis {
  ...
  List<CCRApi> supportedApi() {
    List<CCRApi?> apis = [
      ...rpc.map((e) => e.toSupportedApi(CosmosProviderApi.tendermint)),
      ...rest.map((e) => e.toSupportedApi(CosmosProviderApi.rest)),
      ...grpc.map((e) => e.toSupportedApi(CosmosProviderApi.grpc)),
    ];
    return apis.whereType<CCRApi>().toList();
  }
}
